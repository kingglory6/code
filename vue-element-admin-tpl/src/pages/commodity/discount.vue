    ...
    <el-dialog title="添加活动" :visible.sync="dialogVisible" width="50%">
      <el-form>
        ...
        <el-form-item label="请选择需要参与活动的商品">
          <el-select v-model="discountCommodity.commodity.id" placeholder="请选择" @change="changCommodity()" >
            <el-option
        ...
        <el-form-item label="起始时间">
          <el-date-picker
            v-model="discountCommodity.startTime"
            type="datetime"
        ...
        <el-form-item label="结束时间">
          <el-date-picker
            v-model="discountCommodity.endTime"
            value-format="yyyy-MM-dd HH:mm:ss"
            type="datetime"
            placeholder="选择日期时间"
          ></el-date-picker>
        ...
        <el-form-item label="折扣价">
          <el-input v-model="discountCommodity.price" placeholder="请输入" style="width:30%"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    // 搜索商品
    search() {
      this.discountCommodity.commodity = {};
      this.axios.get(this.common.httpAdminUrl + "/commodity/querycommodity/1", {
          params: {
            size: 0,
            shelf: -1,
            cid: -1,
    ...
    // 选择商品
    changCommodity(){
        this.axios.get(this.common.httpAdminUrl + "/commodity/loadcomm", {
          params: {
            id:this.discountCommodity.commodity.id
          }
        })
        .then(res => {
          this.discountCommodity.price = res.data.price;
          this.discountCommodity.stock = res.data.stock;
        })
        .catch(err => {
          console.error(err);
        });
